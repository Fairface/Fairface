...
contract FAFEcosystemVesting is ReentrancyGuard {
    ...
    uint256 public constant lockTime = 30 days;
     ...
     */
    constructor(IBEP20 _token, uint256 _start, uint256 _end, uint256 _cliffDurationInSecs, address _beneficiary)  {
        require(address(_token) != address(0), "VestingContract::constructor: Invalid token");
        require(_start > block.timestamp, "VestingContract::constructor: Start time should not be less than current time");
    ...
    function createVestingSchedule(uint _amount) public {
        require(msg.sender == owner, "VestingContract::createVestingSchedule: Only owner");
        _createVestingSchedule(beneficiary, _amount);
    }
    ...
    function _transferBalanceToOwner(uint _amount) public {
        require(msg.sender == owner, "VestingContract::_transferBalanceToOwner: Only Owner");
        require(token.transfer(owner, _amount), "VestingContract::_transferBalanceToOwner: Unable to transfer tokens");
    }
}
