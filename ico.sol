...
contract ICO is Ownable, ReentrancyGuard{
    ...
    constructor(uint rate, address payable wallet, IBEP20 token, IBEP20 usdt) {
     ...
     */
    function buyTokens(address referral, address beneficiary, uint256 _amount) public nonReentrant {
        require(!isPaused, "ICO is Paused");
        require(_amount >= minAmount, "Please purchase min amount of GCT");
        ...
        if(!alreadyReferred[msg.sender]){
            if(isValidUser[referral]){
                _processPurchase(referral, tokens.mul(5).div(100));   
            }
        }
